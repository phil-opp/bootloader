...
///
/// In the bootloader, physical memory is identity-mapped, so physical
/// addresses can be used as virtual addresses directly.
pub struct IdentityMappedAccess;

impl kel::PhysicalMemoryAccess for IdentityMappedAccess {
    unsafe fn read_phys(&self, addr: kel::PhysAddr, buf: &mut [u8]) {
    ...
    unsafe fn write_phys(&self, addr: kel::PhysAddr, buf: &[u8]) {
    ...
    unsafe fn zero_phys(&self, addr: kel::PhysAddr, len: usize) {
        let dst = addr.as_u64() as *mut u8;
        unsafe {
    ...
    unsafe fn copy_phys(&self, src: kel::PhysAddr, dst: kel::PhysAddr, len: usize) {
